...
# AccountingStorageHost=$(hostname) vs =localhost). Slurm would otherwise
# honor the last occurrence, but the stale line is confusing and brittle.
set_kv() {
    local kv="$1" key="${1%%=*}"
...
# Use localhost (not $(hostname)) — HyperPod nodes may not have working DNS
# for their own hostname, and slurmdbd binds to localhost (DbdHost=localhost).
set_kv "AccountingStorageHost=localhost"
