...
下記のオプション 2 は、HDF5 デモのディレクトリがすでに手元にあることを前提に
しています。記録自体は**本リポジトリの外**、ローカルにインストールした LeIsaac
で行います。Isaac Sim 環境上で SO-101 をテレオペレーション（キーボード、
SpaceMouse、または実機 SO-101 によるリーダー・フォロワー方式）し、テイク
（タスクへの 1 回の試行。成功・失敗を問いません）を複数回繰り返します。LeIsaac
は各テイクを **エピソード** として HDF5 ファイルに追記していくため、1 つの
HDF5 ファイルには記録セッションを通じて複数のエピソードが蓄積されます。関連
する LeIsaac のドキュメント:

- [Teleoperation](https://lightwheelai.github.io/leisaac/docs/getting_started/teleoperation) — テレオペレーションセッションの起動方法、操作キー、HDF5 ファイルの出力先。
- [Dataset replay](https://lightwheelai.github.io/leisaac/docs/getting_started/dataset_replay) — 記録済み HDF5 から個々のエピソードを再生し、アップロード前に内容を確認する方法。

HDF5 ファイルをディレクトリにまとめたら、オプション 2 と同じ手順でそのディレクトリ
を S3 にアップロードします。HDF5 → LeRobot v2.1 の変換はクラスター側の
`convert` ステージが処理します。
