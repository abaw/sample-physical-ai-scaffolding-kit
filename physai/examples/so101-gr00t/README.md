...
### Recording your own demos in LeIsaac

Option 2 below assumes you already have a directory of HDF5 demonstrations.
Recording itself happens **outside this repo**, in a local LeIsaac install:
you teleoperate the SO-101 in the Isaac Sim environment (keyboard, SpaceMouse,
or a real SO-101 via leader–follower) and run multiple takes — each a single
attempt at the task, success or failure. LeIsaac appends each take as one
**episode** inside an HDF5 file, so a single file accumulates many episodes
across a recording session. The relevant LeIsaac docs:

- [Teleoperation](https://lightwheelai.github.io/leisaac/docs/getting_started/teleoperation) — how to launch a teleop session, control bindings, and where the HDF5 files land.
- [Dataset replay](https://lightwheelai.github.io/leisaac/docs/getting_started/dataset_replay) — replay individual episodes from a recorded HDF5 to spot-check them before uploading.

Once you have your HDF5 file(s) in a directory, upload that directory to S3 as
in Option 2. The `convert` stage handles the HDF5 → LeRobot v2.1 conversion
on the cluster.
